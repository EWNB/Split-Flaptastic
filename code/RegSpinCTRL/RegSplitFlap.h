...
namespace EWNB_RegSplitFlap
{
  ...
  const bool STEPPER_REVERSE_DIR[SREG_NUM_REGS] = {1,0,1};
  const bool STEPPER_ACTIVE_LEVEL[SREG_NUM_REGS] = {1,1,1};
  const bool STEPPER_MICROSTEP = 1;
  const bool STEPPER_ACCELERATE = 1;
  const int STEPPER_STEPS_PER_REV = 2048 * (1+STEPPER_MICROSTEP);
  const int STEPPER_STEP_PERIOD_US = 1700 /(1+STEPPER_MICROSTEP); //1700
  ...
  const int FLAP_NUM_FLAPS[SREG_NUM_REGS] = {32,16,8};
  const int FLAP_HOME_STEP_OFFSET[SREG_NUM_REGS] = {10,10,980 * (1+STEPPER_MICROSTEP)}; //970 1050 1020
